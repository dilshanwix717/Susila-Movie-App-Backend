...
const stripeSession = async (plan) => {
    try {
        const session = await stripe.checkout.sessions.create({
            ...
            success_url: "https://www.slflicks.com/success",
            cancel_url: "https://www.slflicks.com/cancel"
        });
        return session;
    } catch (error) {
...
const activateFreeTrial = async (req, res) => {
    ...
    try {
        const userDocRef = admin.firestore().collection("webAppUsers").doc(customerId);
        const userDocSnap = await userDocRef.get();

        if (userDocSnap.exists) {
            const userData = userDocSnap.data();

            if (userData.subscription?.freeTrialUsed) {
                return res.status(400).json({ error: "Free trial already used." });
            }

            const trialStartDate = moment().format('YYYY-MM-DD');
            const trialEndDate = moment().add(1, 'month').format('YYYY-MM-DD');
            ...
            await userDocRef.update({
                subscription: {
                    ...
                    planType: "Free Trial",
                    status: "active",
                    planStartDate: trialStartDate,
                    planEndDate: trialEndDate,
                },
            });

            res.json({ message: "Free trial activated successfully." });
        } else {
...
const scheduleTrialNotifications = async (userDocRef, trialEndDate) => {
    const userDocSnap = await userDocRef.get();
    ...
    if (userDocSnap.exists) {
        ...
        const oneWeekBefore = moment(trialEndDate).subtract(7, 'days').toISOString();
        const threeDaysBefore = moment(trialEndDate).subtract(3, 'days').toISOString();

        // Schedule notifications
        await emailService.scheduleEmail(email, "Your Free Trial is Ending Soon", "Trial ends in 7 days!", oneWeekBefore);
        await emailService.scheduleEmail(email, "Last Chance to Subscribe", "Trial ends in 3 days!", threeDaysBefore);
    }
};
